(() => {
  ...
  function setKeyboardReference(leftWhiteList=null,rightWhiteList=null) {
    ...
    button.onclick = () => {
      byId('keyboard-map-title').textContent = both ? 'Both hands' : `${leftRange ? 'Left' : 'Right'} hand · ${noteName(activeRange[0])}–${noteName(activeRange[1])}`;
      mapDialog.querySelector('.keyboard-map-description').innerHTML = `${leftRange ? '<span class="map-left">LEFT HAND</span>' : ''}${rightRange ? '<span class="map-right">RIGHT HAND</span>' : ''}`;
